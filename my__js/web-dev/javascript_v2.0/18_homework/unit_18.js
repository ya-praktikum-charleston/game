...
function t4() {
    let t4_span = document.querySelectorAll('span.task-4');
    let out = t4_span.forEach(elem => {
...
function t9() {
    ...
    a9.forEach(function (elem,index) {
        if( index == a9.length-1){
            a9_res += elem.join('-');
        }else{
            a9_res += elem.join('-') + '-';
        }
...
function t10() {
    ...
    for(let key in a10){
        a10_arr.push(key + '=' + a10[key])
    }
    a10_res = 'https://js2.itgid.info?';
