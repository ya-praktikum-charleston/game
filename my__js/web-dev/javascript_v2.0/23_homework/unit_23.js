...
function t3() {
    ...
    document.querySelector('.out-3').innerHTML = txt;
}
...
function t5() {
    ...
    document.querySelector('.out-5').innerHTML = txt;
}
...
let inpRadio = document.querySelectorAll('input[name="rb-9"]');

for (let i = 0; i < inpRadio.length; i++) {
    inpRadio[i].oninput = t9;
}

window.addEventListener('storage', function (e) {
    document.body.style.backgroundColor = localStorage.getItem('value');
});
...
function t11() {
    let arr = localStorage.getItem('card');
    arr = JSON.parse(arr);
    ...
    document.querySelector('.basket tbody').innerHTML = txt;

    let bt = document.querySelectorAll('.basket button');
...
function t12() {
    ...
    let name = this.parentNode.parentNode.children[0].innerHTML;
    let elem = this.parentNode.parentNode.children[1].querySelector('span');
    let col = +elem.innerHTML;
...
function t13() {
    ...
    document.querySelector('.basket tfoot span').innerHTML = sum;
...
function t14() {
    if( localStorage.getItem('card') ){
        document.querySelector('.card_arr').innerHTML = localStorage.getItem('card');
    }else{
        document.querySelector('.card_arr').innerHTML = 'Корзина пуста';
    }
}
